...
void OnTick()
  {
   ...
   if(IsHedgingMode)
     {
      // Scenario #2: Hedging Mode - Volume Reduction Logic
         ...
         if(orderscnt()>0 && (TimeCurrent()-LastCurrentOrderInfo("Time")>=WaitingMinutes*60 || transfair))
           {
            ...
            // If BUY side logic
            if(LastCurrentOrderInfo("Type")==OP_BUY && Bid-LastCurrentOrderInfo("Price")>=PipsToTransfair*point)
              {
               CloseAll(OP_BUY);
               double profit=TotalClosedProfit(T1,OP_BUY);
               ClosePercentOfLoss(OP_SELL,profit); // Step 2: partial close SELL
               reduced=true;
              }
            // If SELL side logic
            else if(LastCurrentOrderInfo("Type")==OP_SELL && LastCurrentOrderInfo("Price")-Ask>=PipsToTransfair*point)
              {
               CloseAll(OP_SELL);
               double profit=TotalClosedProfit(T1,OP_SELL);
               ClosePercentOfLoss(OP_BUY,profit); // Step 2: partial close BUY
               reduced=true;
              }
            ...
            if(reduced)
              {
               transfair=false;
               ExecuteLock(); // Step 3 & 4: Reverse Lock / Balanced hedge
              }
           }
...
void ExecuteLock()
  {
   ...
   if(buy_lots > sell_lots)
     {
      double volume = NormalizeDouble(buy_lots - sell_lots, lot_digits);
      if(volume > 0)
        {
         int ticket = OrderSend(Symbol(), OP_SELL, volume, NormalizeDouble(Bid, Digits), 3*P, 0, 0, "Hedge Lock", MagicNumber, 0, Red);
         if(ticket < 0) Print("ExecuteLock SELL Error: ", GetLastError());
        }
     }
   else if(sell_lots > buy_lots)
     {
      double volume = NormalizeDouble(sell_lots - buy_lots, lot_digits);
      if(volume > 0)
        {
         int ticket = OrderSend(Symbol(), OP_BUY, volume, NormalizeDouble(Ask, Digits), 3*P, 0, 0, "Hedge Lock", MagicNumber, 0, Blue);
