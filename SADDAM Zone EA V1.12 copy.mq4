   ...
   if(IsHedgingMode)
     {
      ...
      // Step 1: Manage Dynamic Secondary Cycles (The New Addition)
...
int SmartOrderSend(int type, double volume, double price, int slip, double sl, double tp, string comment, int magic, int expiration, color arrow)
  {
   ...
   if(type == OP_BUYSTOP && Ask + stopLevel >= price) { finalType = OP_BUY; finalPrice = Ask; }
   else if(type == OP_SELLSTOP && Bid - stopLevel <= price) { finalType = OP_SELL; finalPrice = Bid; }
