      ...
      if(LastCurrentOrderInfo("Type", -1, magic) == OP_BUY && orderscnt_m(magic, OP_SELLSTOP) == 0)
        {
         ...
         if(SmartOrderSend(OP_SELLSTOP, newLot, price, 3 * P, SL, TP, "Grid", magic, 0, Red) < 0)
      ...
      if(LastCurrentOrderInfo("Type", -1, magic) == OP_SELL && orderscnt_m(magic, OP_BUYSTOP) == 0)
        {
         ...
         if(SmartOrderSend(OP_BUYSTOP, newLot, price, 3 * P, SL, TP, "Grid", magic, 0, Blue) < 0)
   ...
   if(lastType == OP_BUY && Bid - lastPrice >= PipsToTransfair * point)
     {
      ...
      ticket = SmartOrderSend(OP_BUY, nl, Ask, 3 * P, SL, TP, "EA", magic, 0, Blue);
      ...
      if(SmartOrderSend(OP_SELLSTOP, NormalizeDouble(nl * Multiplier, lot_digits), pPrice, 3 * P, SL, TP, "EA", magic, 0, Red) < 0)
   ...
   else if(lastType == OP_SELL && lastPrice - Ask >= PipsToTransfair * point)
     {
      ...
      ticket = SmartOrderSend(OP_SELL, nl, Bid, 3 * P, SL, TP, "EA", magic, 0, Red);

      pPrice = LastCurrentOrderInfo("Price", -1, magic) + Step * point + PipsToTransfair * point;
      TP = (TakeProfit != 0) ? pPrice + TakeProfit * point : 0;
      if(StopLoss != 0) SL = pPrice - StopLoss * point;
      if(SmartOrderSend(OP_BUYSTOP, NormalizeDouble(nl * Multiplier, lot_digits), pPrice, 3 * P, SL, TP, "EA", magic, 0, Blue) < 0)
   ...
   if(FirstOrder==BUY)
     {
      TP = (TakeProfit == 0) ? 0 : Ask + TakeProfit * point;
      if(StopLoss != 0) SL = Ask - StopLoss * point;
      if(SmartOrderSend(OP_BUY, newLot, Ask, 3 * P, SL, TP, "Start", magic, 0, Blue) < 0)
   ...
   else
     {
      TP = (TakeProfit == 0) ? 0 : Bid - TakeProfit * point;
      if(StopLoss != 0) SL = Bid + StopLoss * point;
      if(SmartOrderSend(OP_SELL, newLot, Bid, 3 * P, SL, TP, "Start", magic, 0, Red) < 0)
         Print("StartMagicGrid OP_SELL Error: ", GetLastError());
     }
  }

//+------------------------------------------------------------------+
//| SMART ORDER SEND (MARKET CATCH-UP)                               |
//+------------------------------------------------------------------+
int SmartOrderSend(int type, double volume, double price, int slip, double sl, double tp, string comment, int magic, int expiration, color arrow)
  {
   ...
   // 1. Convert Pending to Market if price is already passed
   if(type == OP_BUYSTOP)
     {
      if(Ask + stopLevel >= price)
        {
         finalType = OP_BUY;
         finalPrice = Ask;
         Print("SmartOrderSend: BUYSTOP converted to Market BUY (Price passed or too close)");
        }
     }
   else if(type == OP_SELLSTOP)
     {
      if(Bid - stopLevel <= price)
        {
         finalType = OP_SELL;
         finalPrice = Bid;
         Print("SmartOrderSend: SELLSTOP converted to Market SELL (Price passed or too close)");
        }
     }

   // 2. Validate SL/TP against StopLevel
   double ref = 0;
   ...
   if(ticket < 0) Print("SmartOrderSend Error [", GetLastError(), "] Type:", finalType, " Price:", finalPrice, " TP:", finalTP);
   ...
   if(bl > sl)
     {
      ...
      if(vol >= min_lot) 
         if(SmartOrderSend(OP_SELL, vol, Bid, 3 * P, 0, 0, "Lock", MagicNumber, 0, Red) < 0)
   ...
   else if(sl > bl)
     {
      ...
      if(vol >= min_lot)
         if(SmartOrderSend(OP_BUY, vol, Ask, 3 * P, 0, 0, "Lock", MagicNumber, 0, Blue) < 0)
